...
var populator = require("../services/FilePopulator");

describe("File Populator", function() {
  var record;
  var record2;
  var record3;
  ...
  it("should return the correct total and dupe count", function(done) {
    populator
      .populate({
        all: [file, file2]
      })
      .then(function(fileInfo) {
        expect(fileInfo.all[0]).to.have.property("total", 3);
        expect(fileInfo.all[0]).to.have.property("dupeCount", 1);
        done();
      })
  ...
  it("should return the distinct sources from the file", function(done) {
    populator
      .populate({
        all: [file, file2]
      })
  ...
  it("should return the distinct lists from the file", function(done) {
    populator
      .populate({
        all: [file, file2]
      })
